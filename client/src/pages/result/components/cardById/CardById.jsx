...
export const CardById = ({ selectedItem, closeCard }) => {
  ...
  return (
    <Link
      ref={cardRef}
      key={selectedItem?.id}
      to={`/${i18n.language}/${selectedItem?.id}`}
      className="cardResult"
    >
      <img
        src={
          selectedItem?.photo
            ? `${API_BASE_URL}/images/${selectedItem?.photo} `
            : noImg
        }
      ...
      <div className="cardResult-main">
        ...
        <div className="cardResult-main-bottom">
          <span>
            {roomIcon.icon} {selectedItem?.rooms} {t("room")}
          </span>
          <span>
            {buildType.icon} {selectedItem?.buildingType}
          </span>
          {size === 1 ? (
            <span>
              {square.icon} {selectedItem?.surface} {t("square_symbol")}
            </span>
          ) : (
            <span>
              {square.icon} {sqmToFt2(selectedItem?.surface)} {t("ft_symbol")}
            </span>
          )}
        </div>
      </div>
